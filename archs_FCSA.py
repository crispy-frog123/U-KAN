...
from kan import KANLinear  # 导入KAN相关模块

# --- 引入 archs_1 中的注意力模块 ---
# 注意：你需要确保目录下有 FCS_attention.py 文件，或者将该文件内容复制到这里
from FCS_attention import MultiSpectralAttentionLayer

__all__ = ['UKAN']  # 保持导出列表干净准确
...
class FcsAttention(nn.Module):
    def __init__(self, in_channels, out_channels, img_size, reduction=16):
        ...
        # 动态计算dct尺寸，防止除以0错误或尺寸不匹配
        c2wh = dict(
            [(out_channels // 4, img_size // 4), (out_channels // 2, img_size // 8), (out_channels, img_size // 16)])

        # 简单的容错处理
        dct_h = c2wh.get(out_channels, img_size // 16)
        dct_w = c2wh.get(out_channels, img_size // 16)
...
class UKAN(nn.Module):
    def __init__(self, num_classes, input_channels=3, deep_supervision=False, img_size=224, patch_size=16, in_chans=3,
        ...
        # 优先使用传入的 input_channels，防止硬编码为3
        ...
        # ----------------------------------------------------------------
        # 编码器定义 (采用 archs_1 的全 PatchEmbed + KAN 设计)
        # ----------------------------------------------------------------
        ...
        self.block01 = nn.ModuleList(
            [KANBlock(dim=embed_dims[0] // 8, drop=drop_rate, drop_path=dpr[0], norm_layer=norm_layer)])
        self.block12 = nn.ModuleList(
            [KANBlock(dim=embed_dims[0] // 4, drop=drop_rate, drop_path=dpr[1], norm_layer=norm_layer)])
        self.block23 = nn.ModuleList(
            [KANBlock(dim=embed_dims[0], drop=drop_rate, drop_path=dpr[2], norm_layer=norm_layer)])
        self.block1 = nn.ModuleList(
            [KANBlock(dim=embed_dims[1], drop=drop_rate, drop_path=dpr[3], norm_layer=norm_layer)])
        self.block2 = nn.ModuleList(
            [KANBlock(dim=embed_dims[2], drop=drop_rate, drop_path=dpr[4], norm_layer=norm_layer)])

        # 解码器 KAN Blocks
        self.dblock1 = nn.ModuleList(
            [KANBlock(dim=embed_dims[1], drop=drop_rate, drop_path=dpr[4], norm_layer=norm_layer)])
        self.dblock2 = nn.ModuleList(
            [KANBlock(dim=embed_dims[0], drop=drop_rate, drop_path=dpr[3], norm_layer=norm_layer)])
        self.dblock23 = nn.ModuleList(
            [KANBlock(dim=embed_dims[0] // 4, drop=drop_rate, drop_path=dpr[2], norm_layer=norm_layer)])
        self.dblock12 = nn.ModuleList(
            [KANBlock(dim=embed_dims[0] // 8, drop=drop_rate, drop_path=dpr[1], norm_layer=norm_layer)])
        self.dblock01 = nn.ModuleList(
            [KANBlock(dim=embed_dims[0] // 8, drop=drop_rate, drop_path=dpr[0], norm_layer=norm_layer)])

        # Patch Embedding Layers (用于下采样)
        # 修复：这里的 in_chans 使用传入的 input_channels，而不是写死的 3
        ...
        # FCSA 注意力模块 (Skip Connection Fusion)
    ...
    def forward(self, x):
        ...
        # --- Decoder (Concat + Attention Fusion) ---
        ...
        # --- Multi-scale Feature Aggregation (Final Stage) ---

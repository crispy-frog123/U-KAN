...
class FcsAttention(nn.Module):
    ...
    def _em_prior(self, x):
        ...
        gx = F.conv2d(x, kx, padding=1, groups=self.out_channels)
        gy = F.conv2d(x, ky, padding=1, groups=self.out_channels)
        lap = F.conv2d(x, kl, padding=1, groups=self.out_channels).abs()
        edge = torch.sqrt(gx * gx + gy * gy + 1e-12)
        em_map = (edge + lap).mean(dim=1, keepdim=True)
        em_map = em_map / em_map.amax(dim=(2, 3), keepdim=True).clamp_min(1e-6)
        return em_map

    def forward(self, x):
        x = x * self.channel(x)
        spatial_gate = self.spatial(x)
        em_gate = self._em_prior(x)
